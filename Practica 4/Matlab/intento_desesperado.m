...
% Valores de R2 y nombres de casos
C_v = [1.507904963*10^(-7), 3.626576500*10^(-8), 6.088199105*10^(-7)];  
case_names = {'Critico', 'Subamortiguado', 'Sobreamortiguado'};

% Parámetros de la simulación                
t_cycle = periodo/2;           % Duración de un ciclo (s)
num_cycles = 3;                % Numero de ciclos
...
i = valM
    ...
    for cycle = 0:(num_cycles-1)
        ...
        % Evaluar la solucion analitica en los mismos puntos de tiempo
        t0 = cycle * t_cycle;           % Tiempo inicial del ciclo actual
        Vc0 = initial_conditions(2);    % Voltaje inicial en el capacitor
        Il0 = initial_conditions(1);    % Corriente inicial en el inductor
...
i = valM
    ...
    figure('Name', ['Simulacion RLC -']); 

    % Graficar las respuestas acumuladas
    subplot(4, 1, 1); % Cambiar el layout para mejor visualización
